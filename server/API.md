...
For every endpoint, one question: *can this response be served from a cache
instead of hitting Postgres?* Reads vastly outnumber writes, so caching the
...
single post *and* any feed page it appears on). Drives Week 8 (HTTP cache /
Redis) and Week 9 (CloudFront/CDN).

Note: a `GET` is **not** automatically cacheable — "cacheable" means *the same
response for every caller*. `GET /posts/mine` is per-user, so it can't share a cache.
